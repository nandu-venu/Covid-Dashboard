...
Joined_DF["State_Name"].replace({"Andaman and Nicobar Islands":"Andaman & Nicobar Island","Arunachal Pradesh":"Arunanchal Pradesh",
                               ...
                               "Pondicherry":"Puducherry","Ladakh":"Jammu & Kashmir"},inplace=True)
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.H1("Covid Visualization - India",
    ...
    dbc.Row([
        html.Div(children="Highlights in a date range(use the below date picker)",
                className="text-center mb-2 display-10")
    ]),
    ...
    dcc.Store(id='Joined_DF', data=[], storage_type='memory')  # 'local' or 'session'
...
def update_graph(state):
    if state == 'India':
        ...
        fig3.update_layout(yaxis={'title': 'Cum Confirmed'},
                           title={'text': "Cumulative Confirmed Cases", 'font': {'size': 20}, 'x': 0.5,
                                  'xanchor': 'center'}
                           )
...
def update_graph2(start_date, end_date):
    ...
    map2.update_layout(title={'text': "Total Deaths by State", 'font': {'size': 20}, 'x': 0.5, 'xanchor': 'center'})

    return [map1, map2, Table]
